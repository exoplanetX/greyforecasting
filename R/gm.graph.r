gm.graph<- function(gdata,pattern="fitting"){
  if(pattern=="fitting"){
    ...
    arrows(x[n]+0.2,y[n]*0.6,x[n]-0.9,y[n]*0.6,lty=5,angle=10)
    text(x[n]-1.2,y[n]*0.55,"fitting")
    arrows(x[n]+0.2,y[n]*0.7,x[n]+1,y[n]*0.7,lty=5,angle=10)
    text(x[n]+1,y[n]*0.75,"forecast")
    }
    return(u)
  }
  if(pattern=="errors"){
